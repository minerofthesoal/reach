...
public class ItemGiveScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (qtyField != null && selectedItem != null) {
            if (qtyField.isMouseOver(mouseX, mouseY)) {
                qtyField.mouseClicked(mouseX, mouseY, button);
                setFocused(qtyField);
                return true;
            }
    ...
    public boolean charTyped(char chr, int modifiers) {
        if (qtyField != null) return qtyField.charTyped(chr, modifiers);
    ...
    private void giveSelectedItem() {
        ...
        // 1. Try datapack trigger (works for non-OP players)
        ...
        // 2. Fallback: /give chat command (requires OP)
        if (!sent) {
