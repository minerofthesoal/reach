...
public class EspRenderer {
    ...
    public static void register() {
        HudElementRegistry.addLast(ResourceLocation.fromNamespaceAndPath("reachfly", "esp_renderer"), EspRenderer::renderEsp);
    }
    ...
    private static void renderEsp(GuiGraphics context, DeltaTracker tickCounter) {
        ...
        modelViewStack.mulPose(com.mojang.math.Axis.XP.rotationDegrees(camera.getXRot()));
        modelViewStack.mulPose(com.mojang.math.Axis.YP.rotationDegrees(camera.getYRot() + 180.0f));
        Matrix4f mvMatrix = modelViewStack.last().pose();
        ...
        for (Entity entity : client.level.entitiesForRendering()) {
            ...
            if (ModConfig.espLines) {
                ...
                double ey = entityPos.y - cameraPos.y + entity.getBbHeight() / 2.0;
            ...
            if (ModConfig.espPathTrace) {
                ...
                for (int i = 0; i <= numPoints; i++) {
                    ...
                    double py = findGroundY(client.level, px, playerPos.y, pz);
    ...
    private static double findGroundY(Level world, double x, double startY, double z) {
        ...
        for (int y = sy + 2; y > sy - 10 && y > world.getMinBuildHeight(); y--) {
        ...
        // Search upward
        for (int y = sy; y < sy + 10 && y < world.getMaxBuildHeight(); y++) {
            BlockPos pos = new BlockPos(bx, y, bz);
            BlockPos above = new BlockPos(bx, y + 1, bz);
            if (!world.getBlockState(pos).isAir() && world.getBlockState(above).isAir()) {
                return y + 1;
    ...
    private static boolean shouldShow(Entity entity) {
        ...
        if (entity instanceof Mob && !(entity instanceof AgeableMob)) return ModConfig.espHostile;
        if (entity instanceof AgeableMob) return ModConfig.espPassive;
    ...
    private static int getColor(Entity entity) {
        ...
        if (entity instanceof Mob && !(entity instanceof AgeableMob)) return 0xFFFF8800;
        if (entity instanceof AgeableMob) return 0xFF55FF55;
        return 0xFFFF8800;
    }
