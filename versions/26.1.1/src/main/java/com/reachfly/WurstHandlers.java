...
public class WurstHandlers {
    ...
    private static void tickDolphin(Minecraft client) {
        ...
        if (p.isInWater()) {
            ...
            if (p.isUnderWater() && !p.isSneaking()) {
    ...
    private static void tickAutoSword(Minecraft client) {
        ...
        if (!(client.crosshairTarget instanceof net.minecraft.world.phys.EntityHitResult ehr)) return;
    ...
    private static void tickTriggerBot(Minecraft client) {
        ...
        if (client.crosshairTarget instanceof net.minecraft.world.phys.EntityHitResult ehr) {
    ...
    private static void tickAutoMLG(Minecraft client) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (!client.level.getBlockState(below).isAir()) {
                ...
                client.gameMode.interactItem(p, InteractionHand.MAIN_HAND);
                p.swing(InteractionHand.MAIN_HAND);
    ...
    private static void tickBlink(Minecraft client) {
        ...
        if (blinkTickCounter >= 100) {
            ...
            blinkActive = false;
            blinkStartPos = null;
            p.sendMessage(net.minecraft.network.chat.Component.literal("\u00a7e[Blink] Auto-released (5s limit)"), true);
        }
    }
}
