...
public class AutoArmorHandler {
    ...
    public static void tick(Minecraft client) {
        ...
        for (int s = 0; s < 4; s++) {
            ItemStack current = player.containerMenu.getSlot(slotIndices[s]).getStack();
            ...
            for (int i = 9; i < 45; i++) {
                ItemStack stack = player.containerMenu.getSlot(i).getStack();
            ...
            if (bestInvSlot >= 0) {
                ...
                client.gameMode.handleInventoryMouseClick(syncId, bestInvSlot, 0, ClickType.QUICK_MOVE, player);
