...
public class FlyToCoordsHandler {
    ...
    public static void tick(Minecraft client) {
        ...
        if (horizDist > 3) {
            ...
            boolean blocked = !client.level.getBlockState(ahead1).isAir()
                    || !client.level.getBlockState(ahead2).isAir()
                    || !client.level.getBlockState(ahead1Up).isAir()
                    || !client.level.getBlockState(ahead2Up).isAir();
            ...
            if (stuckTicks > 60 && !client.level.getBlockState(ahead1).isAir()) {
                breaking = BlockBreaker.tryBreak(client, ahead1);
            } else if (stuckTicks > 60 && !client.level.getBlockState(ahead1Up).isAir()) {
